...
# Inicializa o Tkinter para seleção de arquivo
root = tk.Tk()
root.withdraw()
...
with open(log_path, "w", encoding="utf-8") as log:
    log.write(f"LOG DE EXECUCAO - {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}\n")
    log.write("-" * 50 + "\n")
    ...
    for idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        try:
            ...
            # Tenta localizar o campo de data
            ...
            try:
                ...
                # Cria nova linha com evento Arrival at customer
                try:
                    session.FindById("wnd[0]/usr/tabsTAXI_TABSTRIP_HEAD/tabpT\\11/" +
                                     "ssubSUBSCREEN_BODY:SAPMV50A:2122/btnBT_TSEG_INSR").Press()
                    time.sleep(1)
                    session.FindById("wnd[1]/usr/lbl[1,3]").CaretPosition = 16
            ...
            # Trata popup de produtos perigosos, se aparecer
            try:
